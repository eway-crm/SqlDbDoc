<?xml version="1.0" encoding="utf-8"?>
<!-- XSL Template for converting the XML documentation to HTML -->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="msxsl">
  <xsl:output
      method="html"
      indent="yes"
      omit-xml-declaration="yes"
      media-type="text/html"
      doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN"
      doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-strict.dtd"
      encoding="utf-8" />

  <xsl:template match="/">
    <html>
      <head>
        <title>
          <xsl:value-of select="concat(/database/@name, ' Database Schema')" />
        </title>
        <meta http-equiv="content-type" content="text/html; charset=utf-8" />
        <style type="text/css">
          body { background-color: #fff; color: #000; font-family: Consolas, monospace}
          a:link { color: #03d; }
          a:visited { color: #039; }
          a:hover { color: #09f; }
          table { border-collapse: collapse; width: 100%; }
          table caption { color: #666; text-align: right; font-size: 65%; margin-bottom: .5ex; }
          table td, table th { border: solid 1px #666; border-collapse: collapse; padding: .3ex .5ex; vertical-align: top; }
          table tbody tr:hover { background-color: #ffc; }
          table thead th { text-align: left; background-color: #ccc; }
          table tbody th { text-align: left; white-space: nowrap; }
          .table { page-break-inside: avoid; }
          .pk { float: right; cursor: arrow; color: #999; }
          .type { white-space: nowrap; }
          .null { text-align: center; }
          .flag { white-space: nowrap; font-size: 70%; display: inline-block; border: solid 1px #ccc; padding: .25ex .5ex; background-color: #ddd; margin-right: 1ex; }
          .tocref { float: right; text-decoration: none; font-weight: normal; }
          .footer { text-align: center; margin-top: 1em; font-size: 80%; }
          .description { white-space: pre; }
          li.table:before { content: "TABLE"; font-size: 70%; display: inline-block; border: solid 1px #ccc; padding: .25ex .5ex; background-color: #ddd; margin-right: 1ex; }
          li.view:before { content: "VIEW"; font-size: 70%; display: inline-block; border: solid 1px #ccc; padding: .25ex .5ex; background-color: #ddd; margin-right: 1ex; }
          li.procedure:before { content: "PROCEDURE"; font-size: 70%; display: inline-block; border: solid 1px #ccc; padding: .25ex .5ex; background-color: #ddd; margin-right: 1ex; }
          li.scalar_function:before { content: "SCALAR-VALUED FUNCTION"; font-size: 70%; display: inline-block; border: solid 1px #ccc; padding: .25ex .5ex; background-color: #ddd; margin-right: 1ex; }
          li.table_function:before { content: "TABLE-VALUED FUNCTION"; font-size: 70%; display: inline-block; border: solid 1px #ccc; padding: .25ex .5ex; background-color: #ddd; margin-right: 1ex; }
          @media print {
          body { font-size: .8em; }
          #toc, .tocref { display: none; }
          a:link, a:visited { color: #000; text-decoration: none; }
          }
        </style>
      </head>
      <body>
        <h1>
          <xsl:value-of select="concat(/database/@name, ' Database Schema')" />
        </h1>
        <!-- Table of contents -->
        <xsl:call-template name="TableOfContents"/>
        <!-- Process all schemas -->
        <xsl:for-each select="/database/schema">
          <xsl:sort select="@name"/>
          <xsl:variable name="SchemaName" select="@name" />
          <!-- Process tables in schema -->
          <xsl:for-each select="/database/object[@type='USER_TABLE' and @schema=$SchemaName]">
            <xsl:sort select="@name"/>
            <xsl:call-template name="SingleDbObject" />
          </xsl:for-each>
          <xsl:for-each select="/database/object[@type='VIEW' and @schema=$SchemaName]">
            <xsl:sort select="@name"/>
            <xsl:call-template name="SingleDbObject" />
          </xsl:for-each>
          <xsl:for-each select="/database/object[(@type='SQL_STORED_PROCEDURE' or @type='CLR_STORED_PROCEDURE') and @schema=$SchemaName]">
            <xsl:sort select="@name"/>
            <xsl:call-template name="SingleDbObject" />
          </xsl:for-each>
          <xsl:for-each select="/database/object[(@type='SQL_SCALAR_FUNCTION' or @type='SQL_TABLE_VALUED_FUNCTION' or @type='SQL_INLINE_TABLE_VALUED_FUNCTION' or @type='CLR_SCALAR_FUNCTION') and @schema=$SchemaName]">
            <xsl:sort select="@name"/>
            <xsl:call-template name="SingleDbObject" />
          </xsl:for-each>
        </xsl:for-each>
        <!-- Footer -->
        <div class="footer">
          Generated by <a href="http://SqlDbDoc.codeplex.com/">DB&gt;doc</a> on
          <xsl:value-of select="msxsl:format-date(/database/@dateGenerated, 'MMMM dd, yyyy', 'en-US')"/>
          <xsl:value-of select="msxsl:format-time(/database/@dateGenerated, ', HH:mm:ss', 'en-US')"/>
        </div>
      </body>
    </html>
  </xsl:template>

  <xsl:template name="TableOfContents">
    <div id="toc">
      <xsl:choose>
        <xsl:when test="count(/database/schema)=1">
          <!-- Single schema in database -->
          <ul>
            <xsl:for-each select="/database/object[@type='USER_TABLE']">
              <xsl:sort select="@name"/>
              <li class="table">
                <a href="#{@id}">
                  <xsl:value-of select="concat(@schema, '.', @name)"/>
                </a>
              </li>
            </xsl:for-each>
            <xsl:for-each select="/database/object[@type='VIEW']">
              <xsl:sort select="@name"/>
              <li class="view">
                <a href="#{@id}">
                  <xsl:value-of select="concat(@schema, '.', @name)"/>
                </a>
              </li>
            </xsl:for-each>
            <xsl:for-each select="/database/object[(@type='SQL_STORED_PROCEDURE' or @type='CLR_STORED_PROCEDURE')]">
              <xsl:sort select="@name"/>
              <li class="procedure">
                <a href="#{@id}">
                  <xsl:value-of select="concat(@schema, '.', @name)"/>
                </a>
              </li>
            </xsl:for-each>
            <xsl:for-each select="/database/object[(@type='SQL_SCALAR_FUNCTION' or @type='CLR_SCALAR_FUNCTION')]">
              <xsl:sort select="@name"/>
              <li class="scalar_function">
                <a href="#{@id}">
                  <xsl:value-of select="concat(@schema, '.', @name)"/>
                </a>
              </li>
            </xsl:for-each>
            <xsl:for-each select="/database/object[(@type='SQL_TABLE_VALUED_FUNCTION' or @type='SQL_INLINE_TABLE_VALUED_FUNCTION')]">
              <xsl:sort select="@name"/>
              <li class="table_function">
                <a href="#{@id}">
                  <xsl:value-of select="concat(@schema, '.', @name)"/>
                </a>
              </li>
            </xsl:for-each>
          </ul>
        </xsl:when>
        <xsl:otherwise>
          <!-- Multiple schemas in database -->
          <ul>
            <xsl:for-each select="/database/schema">
              <xsl:sort select="@name"/>
              <xsl:variable name="SchemaName" select="@name"/>
              <li>
                <b>
                  <xsl:value-of select="$SchemaName"/>
                </b>
                <ul>
                  <xsl:for-each select="/database/object[@type='USER_TABLE' and @schema=$SchemaName]">
                    <xsl:sort select="@name"/>
                    <li class="table">
                      <a href="#{@id}">
                        <xsl:value-of select="@name"/>
                      </a>
                    </li>
                  </xsl:for-each>
                  <xsl:for-each select="/database/object[@type='VIEW' and @schema=$SchemaName]">
                    <xsl:sort select="@name"/>
                    <li class="view">
                      <a href="#{@id}">
                        <xsl:value-of select="@name"/>
                      </a>
                    </li>
                  </xsl:for-each>
                  <xsl:for-each select="/database/object[(@type='SQL_STORED_PROCEDURE' or @type='CLR_STORED_PROCEDURE')]">
                    <xsl:sort select="@name"/>
                    <li class="procedure">
                      <a href="#{@id}">
                        <xsl:value-of select="concat(@schema, '.', @name)"/>
                      </a>
                    </li>
                  </xsl:for-each>
                  <xsl:for-each select="/database/object[(@type='SQL_SCALAR_FUNCTION' or @type='CLR_SCALAR_FUNCTION')]">
                    <xsl:sort select="@name"/>
                    <li class="scalar_function">
                      <a href="#{@id}">
                        <xsl:value-of select="concat(@schema, '.', @name)"/>
                      </a>
                    </li>
                  </xsl:for-each>
                  <xsl:for-each select="/database/object[(@type='SQL_TABLE_VALUED_FUNCTION' or @type='SQL_INLINE_TABLE_VALUED_FUNCTION')]">
                    <xsl:sort select="@name"/>
                    <li class="table_function">
                      <a href="#{@id}">
                        <xsl:value-of select="concat(@schema, '.', @name)"/>
                      </a>
                    </li>
                  </xsl:for-each>
                </ul>
              </li>
            </xsl:for-each>
          </ul>
        </xsl:otherwise>
      </xsl:choose>
    </div>
  </xsl:template>

  <xsl:template name="SingleDbObject">
    <div class="table">
      <h2 id="{@id}">
        <a href="#toc" class="tocref">&#8679;</a>
        <label style="font-weight: normal">
          <xsl:choose>
            <xsl:when test="@type='USER_TABLE'">Table </xsl:when>
            <xsl:when test="@type='VIEW'">View </xsl:when>
            <xsl:when test="(@type='SQL_STORED_PROCEDURE' or @type='CLR_STORED_PROCEDURE')">Stored Procedure </xsl:when>
            <xsl:when test="(@type='SQL_SCALAR_FUNCTION' or @type='CLR_SCALAR_FUNCTION')">Scalar-Valued Function </xsl:when>
            <xsl:when test="(@type='SQL_TABLE_VALUED_FUNCTION' or @type='SQL_INLINE_TABLE_VALUED_FUNCTION')">Table-Valued Function </xsl:when>
          </xsl:choose>
        </label>
        <xsl:value-of select="concat(@schema, '.', @name)"/>
      </h2>

      <xsl:if test="@description">
        <p class="description">
          <xsl:value-of select="@description"/>
        </p>
      </xsl:if>

      <xsl:if test="@type='USER_TABLE' or @type='VIEW'">
        <table>
          <caption>
            <xsl:value-of select="concat('Object ID: ', @id)"/>
            <xsl:value-of select="concat('; Created: ', msxsl:format-date(@dateCreated, 'MMMM dd, yyyy', 'en-US'), msxsl:format-time(@dateCreated, ', HH:mm:ss', 'en-US'))" />
            <xsl:if test="@dateCreated != @dateModified">
              <xsl:value-of select="concat('; Modified: ', msxsl:format-date(@dateModified, 'MMMM dd, yyyy', 'en-US'), msxsl:format-time(@dateModified, ', HH:mm:ss', 'en-US'))" />
            </xsl:if>
          </caption>
          <thead>
            <tr>
              <th style="width:30ex">Name</th>
              <th style="width:20ex">Type</th>
              <th style="width:4ex">NULL</th>
              <th>Comment</th>
            </tr>
          </thead>
          <tbody>
            <xsl:for-each select="column">
              <tr>
                <th>
                  <xsl:if test="primaryKey">
                    <xsl:variable name="PK" select="primaryKey" />
                    <span class="pk" title="PRIMARY KEY {//object[@id=$PK/@refId]/@name}">&#9670;</span>
                  </xsl:if>
                  <xsl:value-of select="@name"/>
                </th>
                <td class="type">
                  <xsl:value-of select="@type"/>
                  <xsl:choose>
                    <xsl:when test="@length=-1"> (max)</xsl:when>
                    <xsl:when test="@type='char' or @type='varchar' or @type='binary' or @type='varbinary'">
                      <xsl:value-of select="concat(' (', @length, ')')"/>
                    </xsl:when>
                    <xsl:when test="@type='nchar' or @type='nvarchar'">
                      <xsl:value-of select="concat(' (', @length div 2, ')')"/>
                    </xsl:when>
                    <xsl:when test="@type='real' or @type='money' or @type='float' or @type='decimal' or @type='numeric' or @type='smallmoney'">
                      <xsl:value-of select="concat(' (', @precision, ', ', @scale, ')')"/>
                    </xsl:when>
                  </xsl:choose>
                </td>
                <td class="null">
                  <xsl:choose>
                    <xsl:when test="@nullable='true'">&#9746;</xsl:when>
                    <xsl:otherwise>&#9744;</xsl:otherwise>
                  </xsl:choose>
                </td>
                <td>
                  <xsl:if test="@identity='true'">
                    <span class="flag">IDENTITY</span>
                  </xsl:if>
                  <xsl:if test="@computed='true'">
                    <span class="flag">COMPUTED</span>
                  </xsl:if>
                  <xsl:if test="default">
                    <span class="flag">
                      <xsl:value-of select="concat('DEFAULT ', default/@value)"/>
                    </span>
                  </xsl:if>
                  <xsl:if test="foreignKey">
                    <xsl:variable name="FK" select="foreignKey" />
                    <span class="flag" title="{//object[@id=$FK/@refId]/@name}">
                      <xsl:text>&#8680; </xsl:text>
                      <a href="#{$FK/@tableId}">
                        <xsl:value-of select="concat(//object[@id=$FK/@tableId]/@schema, '.', //object[@id=$FK/@tableId]/@name)"/>
                      </a>
                      <xsl:value-of select="concat('.', foreignKey/@column)"/>
                    </span>
                  </xsl:if>
                  <xsl:value-of select="@description"/>
                </td>
              </tr>
            </xsl:for-each>
          </tbody>
        </table>
      </xsl:if>

      <xsl:if test="(@type='SQL_STORED_PROCEDURE' or @type='CLR_STORED_PROCEDURE' or @type='SQL_SCALAR_FUNCTION' or @type='SQL_TABLE_VALUED_FUNCTION' or @type='SQL_INLINE_TABLE_VALUED_FUNCTION' or @type='CLR_SCALAR_FUNCTION') and count(parameter) > 1">
        <table>
          <caption>
            <xsl:value-of select="concat('Object ID: ', @id)"/>
            <xsl:value-of select="concat('; Created: ', msxsl:format-date(@dateCreated, 'MMMM dd, yyyy', 'en-US'), msxsl:format-time(@dateCreated, ', HH:mm:ss', 'en-US'))" />
            <xsl:if test="@dateCreated != @dateModified">
              <xsl:value-of select="concat('; Modified: ', msxsl:format-date(@dateModified, 'MMMM dd, yyyy', 'en-US'), msxsl:format-time(@dateModified, ', HH:mm:ss', 'en-US'))" />
            </xsl:if>
          </caption>
          <thead>
            <tr>
              <th style="width:30ex">Name</th>
              <th style="width:20ex">Type</th>
              <th style="width:20ex">Default</th>
              <th>Comment</th>
            </tr>
          </thead>
          <tbody>
            <xsl:for-each select="parameter">
              <tr>
                <th>
                  <xsl:value-of select="@name"/>
                  <xsl:if test="@is_output='true'"> OUTPUT</xsl:if>
                </th>
                <td class="type">
                  <xsl:value-of select="@type"/>
                  <xsl:choose>
                    <xsl:when test="@length=-1"> (max)</xsl:when>
                    <xsl:when test="@type='char' or @type='varchar' or @type='binary' or @type='varbinary'">
                      <xsl:value-of select="concat(' (', @length, ')')"/>
                    </xsl:when>
                    <xsl:when test="@type='nchar' or @type='nvarchar'">
                      <xsl:value-of select="concat(' (', @length div 2, ')')"/>
                    </xsl:when>
                    <xsl:when test="@type='real' or @type='money' or @type='float' or @type='decimal' or @type='numeric' or @type='smallmoney'">
                      <xsl:value-of select="concat(' (', @precision, ', ', @scale, ')')"/>
                    </xsl:when>
                  </xsl:choose>
                </td>
                <td>
                  <xsl:value-of select="@defalt_value"/>
                </td>
                <td>
                  <xsl:value-of select="@description"/>
                </td>
              </tr>
            </xsl:for-each>
          </tbody>
        </table>
      </xsl:if>
    </div>
  </xsl:template>

</xsl:stylesheet>
